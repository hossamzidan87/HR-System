        ...
        foreach ($selected_employees as $employee_code) {
            ...
            if ($row['count'] == 0) {
                ...
                for ($i = 0; $i < 5; $i++) {
                    ...
                    $overtime_sql = "INSERT INTO overtime (employee_code, employee_name, department, job, bus_line_name, processed_by, processed_at, overtime_date, type)
                                     VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, 'night')";
        ...
        if ($selected_department === 'all') {
            ...
            if ($stmt) {
                ...
                for ($i = 0; $i < 5; $i++) {
                    ...
                    $stmt = $conn->prepare($overtime_sql);
                    if ($stmt) {
                        $stmt->bind_param("sss", $username, $overtime_date, $copy_to_start_date);
                        $stmt->execute();
                        $stmt->close();
                    }
                    $current_date = strtotime('+1 day', $current_date);
            ...
            if ($stmt) {
                ...
                for ($i = 0; $i < 5; $i++) {
                    ...
                    $stmt = $conn->prepare($overtime_sql);
                    if ($stmt) {
                        $stmt->bind_param("ssss", $username, $overtime_date, $copy_to_start_date, $selected_department);
                        $stmt->execute();
                        $stmt->close();
                    }
                    $current_date = strtotime('+1 day', $current_date);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_all'])) {
    ...
    if (!empty($start_date) && !empty($end_date)) {
        ...
        if ($stmt) {
            ...
            $stmt->execute();
            $stmt->close();
            echo "<p>All employees successfully deleted for the selected week.</p>";
        } else {
            echo "<p>Error preparing statement: " . $conn->error . "</p>";
        }
    } else {
...
<form method="POST" action="night_shift_test.php" id="filterForm">
...
<?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && $start_date && $end_date): ?>
    ...
    ?>
    <form method="POST" action="night_shift_test.php">
