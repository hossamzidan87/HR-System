...
function setupBookingForm() {
    document.getElementById('booking-date').addEventListener('change', function() {
        if (selectedRoomId) {
            ...
            // Recalculate recurrence end dates when start date changes
    ...
    document.getElementById('recurrence-type').addEventListener('change', function() {
        handleRecurrenceTypeChange(this.value);
    });
...
function handleRecurrenceTypeChange(type) {
    document.getElementById('daily-options').classList.add('hidden');
    document.getElementById('weekly-options').classList.add('hidden');
    document.getElementById('monthly-options').classList.add('hidden');
    
    const startDateStr = document.getElementById('booking-date').value;
    
    if (type === 'daily') {
        document.getElementById('daily-options').classList.remove('hidden');
        // Daily: 1 week = 7 days
        const endDate = new Date(startDateStr);
        endDate.setDate(endDate.getDate() + 6); // +6 to include start date (total 7 days)
        ...
        // Weekly: 1 month = 4 weeks = 28 days
        const endDate = new Date(startDateStr);
        endDate.setDate(endDate.getDate() + 27); // +27 to include start date (total 28 days/4 weeks)
        document.getElementById('weekly-end-date').value = endDate.toISOString().split('T')[0];
        // Pre-select the current day of week
        const currentDay = new Date(startDateStr).getDay();
        document.querySelector(`input[name="day"][value="${currentDay}"]`).checked = true;
    } else if (type === 'monthly') {
        document.getElementById('monthly-options').classList.remove('hidden');
        // Monthly: 1 year = 12 months
        const endDate = new Date(startDateStr);
        endDate.setFullYear(endDate.getFullYear() + 1);
        document.getElementById('monthly-end-date').value = endDate.toISOString().split('T')[0];
    }
}
...
function submitBooking() {
    ...
    let datesToBook = [startDate];
    
    // Generate recurring dates
    if (recurrenceType === 'daily') {
        const endDate = document.getElementById('daily-end-date').value;
        if (!endDate) {
            showMessage('Please select an end date for daily recurrence', 'error', messageDiv);
            return;
        }
        datesToBook = generateDailyDates(startDate, endDate);
        ...
        if (selectedDays.length === 0) {
            showMessage('Please select at least one day of the week', 'error', messageDiv);
            return;
        }
        if (!endDate) {
            showMessage('Please select an end date for weekly recurrence', 'error', messageDiv);
            return;
        }
        datesToBook = generateWeeklyDates(startDate, endDate, selectedDays);
    } else if (recurrenceType === 'monthly') {
        const endDate = document.getElementById('monthly-end-date').value;
        if (!endDate) {
            showMessage('Please select an end date for monthly recurrence', 'error', messageDiv);
            return;
        }
        datesToBook = generateMonthlyDates(startDate, endDate);
    }
    
    // Show how many dates will be booked
    if (datesToBook.length > 1) {
        showMessage(`Booking ${datesToBook.length} dates...`, 'info', messageDiv);
    }
    ...
    formData.append('dates', JSON.stringify(datesToBook));
    ...
    .then(data => {
        if (data.success) {
            const message = `Successfully booked ${data.booked_count} date(s)`;
            const additional = data.conflict_count > 0 ? ` (${data.conflict_count} date(s) had conflicts)` : '';
            showMessage(message + additional, 'success', messageDiv);
...
function generateWeeklyDates(startDate, endDate, selectedDays) {
    const dates = [];
    let current = new Date(startDate);
    const end = new Date(endDate);
...
function generateMonthlyDates(startDate, endDate) {
    ...
    while (current <= end) {
        // Try to set to the same day of month
    ...
    // Add the start date if not already included
